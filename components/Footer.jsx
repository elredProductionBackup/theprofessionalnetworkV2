...
const Footer = () => {
  return (
    <footer className="bg-white border-t flex items-center justify-center flex-col border-zinc-100 font-inter ">
      {/* These popups are no longer triggered from the footer (Privacy/Terms now route
          to real pages). Left mounted in case they're opened elsewhere — safe to remove
          if nothing else dispatches these events. */}
      <PrivacyPolicyPopup eventName="openPrivacyPopup" />
      <ComingSoonPopup eventName="openTermsPopup" />
      <ComingSoonPopup eventName="openDisclaimerPopup" />
      <div className="w-full md:pt-0 text-[#333336]">
...
<div className="lg:col-span-2 flex w-full gap-[140px] px-[24px] md:pl-[40px] border-b border-[#E6E6E6] md:border-b-0 md:border-r ">
  ...
  <div className="w-auto md:space-y-4 space-y-[16px] py-[24px]">
    ...
    <FooterLink text="Privacy Policy" />
    <FooterLink text="Shipping & Exchange" />
    <FooterLink text="Cancellation & Refund" />
...
const FooterLink = ({ text }) => {
  ...
  const handleClick = () => {
    ...
    if (text === "Governing Charter") {
      router.push("/governing-charter");
      return;
    }
    ...
    if (text === "Shipping & Exchange") {
      router.push("/shipping-and-exchange");
      return;
    }

    if (text === "Cancellation & Refund") {
      router.push("/cancellation-and-refund");
      return;
    }
