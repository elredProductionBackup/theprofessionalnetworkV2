...
/* query param that, when present in the URL, auto-opens the agenda popup */
...
export default function EventRegistration() {
  ...
  /* if someone arrives via a shared link (?details=open), auto-open the agenda */
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get(SHARE_PARAM) === SHARE_VALUE) setDescOpen(true);
  }, []);

  /* build a link that carries the auto-open param, then fire the native share sheet
     (falls back to copying the link on browsers without the Web Share API) */
  const handleShare = async () => {
    ...
    // no native share (most desktops) → copy the link instead
    try {
          ...
          <div className="relative z-10 flex max-h-[90vh] w-full max-w-[700px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="absolute right-4 top-4 z-20 flex items-center gap-2">
              <button
                ...
                onClick={handleShare}
                aria-label="Share this event"
                className="grid h-9 w-9 place-items-center rounded-full bg-rose-100 transition-colors hover:bg-rose-200"
                style={{ color: RED }}
              >
                {copied ? (
            ...
            <div className="border-t border-slate-100 bg-white px-6 py-4 sm:px-8">
              <div className="flex items-center gap-3">
                <button
                  ...
                  onClick={handleShare}
                  aria-label="Share this event"
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full px-5 py-3.5 text-base font-semibold ring-1 ring-rose-200 transition hover:bg-rose-50"
                  style={{ color: RED }}
                >
                  {copied ? (
                    <Check className="h-5 w-5" />
                ...
                <button
                  type="button"
                  onClick={openApply}
                  className="flex-1 rounded-full py-3.5 text-base font-semibold text-white transition hover:opacity-90"
                  style={{ backgroundColor: RED }}
                >
                  Register
                </button>
